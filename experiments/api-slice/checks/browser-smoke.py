...
try:
    ...
    browser("fill", "#member-project", "41")
    submit("403 · Owner permission required")
    capture("iris-member-forbidden")
    browser("select", "#identity", "11")
    browser("eval", "window.irisTest.hold = true; window.irisTest.requests = 0")
    submit("Waiting for the API")
    browser("eval", "document.querySelector('form').requestSubmit()")
    check("window.irisTest.requests === 1 && document.querySelector('fieldset').disabled")
    capture("iris-member-loading")
    browser("eval", "window.irisTest.hold = false; window.irisTest.release()")
    browser("wait", "--text", "200 · Member role changed")
    check("JSON.parse(document.querySelector('pre').textContent).role === 'viewer'")
    capture("iris-member-role")
    browser("fill", "#member", "11")
    submit("409 · Last owner must remain")
    capture("iris-member-last-owner")
    browser("fill", "#member", "29")
    browser("select", "#role", "owner")
    submit("200 · Member role changed")
    browser("fill", "#member", "11")
    ...
    browser("select", "#identity", "29")
    check("!document.querySelector('input[type=checkbox]').checked && document.querySelector('button[type=submit]').disabled")
    browser("check", "input[type=checkbox]")
    browser("eval", "window.irisTest.hold = true; window.irisTest.requests = 0")
    submit("Waiting for the API")
    browser("eval", "document.querySelector('form').requestSubmit()")
    ...
    check("JSON.parse(document.querySelector('pre').textContent).role === null && document.querySelector('button[type=submit]').disabled")
    capture("iris-member-removed")
    browser("check", "input[type=checkbox]")
    submit("404 · Member not found")
    browser("fill", "#member", "29")
    ...
    browser("set", "viewport", "390", "844", "2")
    check("document.documentElement.scrollWidth === innerWidth")
    capture("iris-member-remove-narrow")
    browser("eval", "window.irisTest.fail = true; window.irisTest.requests = 0")
    submit("Couldn’t read an API response")
    check("window.irisTest.requests === 1 && !document.querySelector('fieldset').disabled")
    print("PASS: invitation workflow; member role changes, promotion, self-demotion, removal confirmation/reset, duplicate-submit guards, last-owner conflicts, authorization, not-found, network failure, narrow layout")
finally:
    browser("close")
